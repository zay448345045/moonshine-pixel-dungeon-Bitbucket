 ...
 */
package com.moonshinepixel.moonshinepixeldungeon.mechanics;
...
public class Ballistica {
	...
	public Integer collisionPos2 = null;
	...
	public static final int GUNBULLET =    STOP_CHARS  | STOP_TERRAIN;
	...
	private void build( int from, int to, boolean stopTarget, boolean stopChars, boolean stopTerrain ) {
		...
		while (Dungeon.level.insideMap(cell)) {
			...
			if (stopTerrain && cell != sourcePos && !Level.getPassable(cell) && !Level.getAvoid(cell)) {
			...
			if ((stopTerrain && cell != sourcePos && Level.getLosBlocking(cell))
	...
	private void collide2(int cell){
		if (collisionPos2 == null)
			collisionPos2 = cell;
	}
