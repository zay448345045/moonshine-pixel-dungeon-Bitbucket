...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		int slots = MoonshinePixelDungeon.quickSlots();
		...
		for(int i = 0; i <= 3; i++) {
			...
			if (slots == 4 && width < 150){
				if (width < 139){
					if ((MoonshinePixelDungeon.flipToolbar() && i == 3) ||
							(!MoonshinePixelDungeon.flipToolbar() && i == 0)) {
					...
					if (i == 0 && !MoonshinePixelDungeon.flipToolbar() ||
							i == 3 && MoonshinePixelDungeon.flipToolbar()){
						btnQuick[i].border(0, 2);
						btnQuick[i].frame(106, 0, 19, 24);
					} else if (i == 0 && MoonshinePixelDungeon.flipToolbar() ||
							i == 3 && !MoonshinePixelDungeon.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(MoonshinePixelDungeon.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
		...
		if (MoonshinePixelDungeon.flipToolbar()) {
