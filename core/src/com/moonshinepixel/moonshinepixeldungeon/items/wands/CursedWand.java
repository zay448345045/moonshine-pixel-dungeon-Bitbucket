...
public class CursedWand {
	...
	private static void rareEffect(final Wand wand, final Hero user, final Ballistica bolt){
		switch(Random.Int(4)){
			...
			case 2:
				/*if (Dungeon.fakedepth[Dungeon.depth] > 1 && !Dungeon.bossLevel()) {

					//each depth has 1 more weight than the previous depth.
					float[] depths = new float[Dungeon.fakedepth[Dungeon.depth]-1];
					for (int i = 1; i < Dungeon.fakedepth[Dungeon.depth]; i++) depths[i-1] = i;
			...
			case 1:
				cursedFX(user, bolt, new Callback() {
					public void call() {
						...
						mimic.adjustStats(Dungeon.fakedepth[Dungeon.depth] + 10);
						mimic.HP = mimic.HT;
						Item reward;
						do {
