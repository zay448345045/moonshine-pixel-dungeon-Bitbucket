...
public class WndLangs extends Window {
	...
	public WndLangs(){
		...
		for (int i = 0; i < langs.size(); i++){
			...
			RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName())){
				...
				protected void onClick() {
					...
					MoonshinePixelDungeon.switchNoFade(TitleScene.class, new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							//MoonshinePixelDungeon.language(langs.get(langIndex));
			...
			if (MoonshinePixelDungeon.landscape() && i % 2 == 1){
		...
		resize(MoonshinePixelDungeon.landscape() ? WIDTH_L : WIDTH_P, y);
